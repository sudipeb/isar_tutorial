...
import 'package:isartutorial/isar_service.dart';
import 'package:isartutorial/user.dart';
import 'package:path_provider/path_provider.dart';

late Isar isar;
Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  final dir = await getApplicationDocumentsDirectory(); // Get device storage path
  isar = await Isar.open(
    [UserSchema], // List of schemas
    directory: dir.path,
  );
  runApp(const MyApp());
}
                                    ...
                                    children: [
                                      ...
                                      InkWell(
                                        onTap: () {
                                          ...
                                          _controller.text = snapshot.data![index].name!;
                                          _controller2.text = snapshot.data![index].id.toString();
                                          showModel("update", snapshot.data![index]);
                                        },
                                        child: Icon(Icons.autorenew_outlined, size: 30),
                                      ),
                                      SizedBox(width: 10),
                                      // Delete user icon
                                      InkWell(
                                        onTap: () async {
                                          // Delete user
                                          await isarService.deleteUser(snapshot.data![index].id);
                                        },
                            ...
                            ? Padding(
                                ...
                                child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    await isarService.saveUser(User(_controller.text, int.parse(_controller2.text)));
                                    Navigator.of(context).pop();
                                    _controller.text = "";
                                    _controller2.text = "";
                                  },
